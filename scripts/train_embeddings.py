...
def train_embeddings() -> None:
    ...
    batch_size = 8
    epochs = 1
    learning_rate = 1e-5  # Reduced from 3e-5 to prevent catastrophic forgetting
    ...
    # Calculate warmup steps (10% of data)
    warmup_steps = int(len(train_dataloader) * epochs * 0.1)
    
    model.fit(
        train_objectives=[(train_dataloader, train_loss)],
        epochs=epochs,
        weight_decay=0.01,     # Added slight weight decay
        scheduler="warmupcosine", # Changed to warmupcosine for stability
